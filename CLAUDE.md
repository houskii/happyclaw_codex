...
- **MCP Server**：11 个工具（`send_message`、`schedule_task`、`list/pause/resume/cancel_task`、`register_group`、`install_skill`、`uninstall_skill`、`memory_query`、`memory_remember`），通过 SDK `createSdkMcpServer()` 以同进程模式注册
...
| `mcp-tools.ts` | MCP 工具定义：11 个工具通过 SDK `tool()` 注册，IPC 文件通信 |
